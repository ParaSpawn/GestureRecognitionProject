...
def write_results(prediction, confidence, num_classes, nms_conf=0.4):
    ...
    # conf_mask : 1 * n * 1, checks if object is  present 1 = YES
    prediction = prediction*conf_mask
    # prediction 1 * n * 85, all rows that do not have object are now set to 0
    ...
    for ind in range(batch_size):
       ...
       # NMS
        max_conf, max_conf_ind = torch.max(image_pred[:, 5:5 + num_classes], 1)
        # max_conf : n, gives the maximum confidence in the row
        max_conf = max_conf.float().unsqueeze(1)
        # n * 1
        max_conf_ind = max_conf_ind.float().unsqueeze(1)
        # n * 1
        seq = (image_pred[:, :5], max_conf, max_conf_ind)
        image_pred = torch.cat(seq, 1)
        # n * (4 + 1 + 1)
        non_zero_ind = (torch.nonzero(image_pred[:, 4]))
        # gives indices of inputs where confidence is non zero
        try:
